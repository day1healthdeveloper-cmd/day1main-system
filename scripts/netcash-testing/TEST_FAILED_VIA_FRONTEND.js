/**
 * Test failed payments via frontend API route
 */
...
async function testFailedPayments() {
  console.log('🧪 Testing Failed Payments via Frontend API...\n');
  
  try {
    // Test 1: Get failed payments list
    console.log('1️⃣ Testing GET /api/netcash/failed-payments');
    const listResponse = await fetch(`${FRONTEND_URL}/api/netcash/failed-payments?limit=10`, {
      headers: {
    ...
    if (listResponse.ok) {
      ...
      if (data.transactions && data.transactions.length > 0) {
        ...
        data.transactions.slice(0, 3).forEach(txn => {
          console.log(`     - ${txn.member_number}: R${txn.amount} (${txn.rejection_reason || 'Unknown reason'})`);
        });
      }
      console.log('');
    } else {
    ...
    console.log('2️⃣ Testing GET /api/netcash/failed-payments/stats/summary');
    const statsResponse = await fetch(`${FRONTEND_URL}/api/netcash/failed-payments/stats/summary`, {
      headers: {
        'Authorization': `Bearer ${ACCESS_TOKEN}`,
        'Content-Type': 'application/json'
      }
    ...
    console.log('\n💡 Now refresh the browser at http://localhost:3001/operations/debit-orders');
    console.log('   and click on the "Failed" tab to see the failed payments.');
