...
export class PreAuthService {
  ...
  async submitPreAuthRequest(policyId: string, memberId: string, providerId: string, serviceType: string, diagnosisCode: string, procedureCodes: string[], estimatedCost: number, userId: string): Promise<PreAuthRequest> {
    ...
    const { data: policy } = await this.supabase.from('policies').select('*').eq('id', policyId).single()
  ...
  private async generatePreAuthNumber(): Promise<string> {
    ...
    const { count } = await this.supabase.from('preauth_requests').select('*', { count: 'exact', head: true }).gte('submitted_at', startOfDay)
  ...
  async approvePreAuth(preauthId: string, approvedAmount: number, conditions: any, expiryDate: Date, userId: string): Promise<PreAuthDecision> {
    const { data: preauth } = await this.supabase.from('preauth_requests').select('*').eq('id', preauthId).single()
    ...
    const { data: decision, error } = await this.supabase.from('preauth_decisions').insert({ preauth_id: preauthId, decision: 'approved', approved_amount: approvedAmount, conditions, expiry_date: expiryDate.toISOString(), decided_by: userId }).select().single()
    if (error) throw new BadRequestException('Failed to create decision')

    await this.supabase.from('preauth_requests').update({ status: 'approved' }).eq('id', preauthId)
  ...
  async rejectPreAuth(preauthId: string, reason: string, userId: string): Promise<PreAuthDecision> {
    ...
    const { data: decision, error } = await this.supabase.from('preauth_decisions').insert({ preauth_id: preauthId, decision: 'rejected', conditions: { reason }, decided_by: userId }).select().single()
    if (error) throw new BadRequestException('Failed to create decision')

    await this.supabase.from('preauth_requests').update({ status: 'rejected' }).eq('id', preauthId)
  ...
  async getPreAuthsByStatus(status: string): Promise<PreAuthRequest[]> {
    const { data } = await this.supabase.from('preauth_requests').select('*').eq('status', status).order('submitted_at', { ascending: false })
  ...
  async getPreAuthsByMember(memberId: string): Promise<PreAuthRequest[]> {
    const { data } = await this.supabase.from('preauth_requests').select('*').eq('member_id', memberId).order('submitted_at', { ascending: false })
    return (data || []).map((p) => ({ id: p.id, preauth_number: p.preauth_number, policy_id: p.policy_id, member_id: p.member_id, provider_id: p.provider_id, service_type: p.service_type, diagnosis_code: p.diagnosis_code, procedure_codes: p.procedure_codes, estimated_cost: parseFloat(p.estimated_cost), status: p.status, submitted_at: p.submitted_at }))
  }
