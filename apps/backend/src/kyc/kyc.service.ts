...
export class KycService {
  ...
  async performKyc(memberId: string, dto: PerformKycDto, userId: string) {
    const { data: member } = await this.supabase.from('members').select('*').eq('id', memberId).single()
    ...
    const { data: updated } = await this.supabase.from('members').update({ kyc_status: idVerification.verified ? 'completed' : 'failed', risk_score: riskScore, pep_status: pepStatus }).eq('id', memberId).select().single()
  ...
  async updateRiskScore(memberId: string, dto: UpdateRiskScoreDto, userId: string) {
    ...
    const { data: updated } = await this.supabase.from('members').update({ risk_score: dto.risk_score }).eq('id', memberId).select().single()
  ...
  async flagRisk(memberId: string, dto: FlagRiskDto, userId: string) {
    ...
    const { data: flag, error } = await this.supabase.from('member_risk_flags').insert({ member_id: memberId, flag_type: dto.flag_type, flag_reason: dto.reason, severity: dto.severity, flagged_by: userId }).select().single()
  ...
  async resolveRiskFlag(flagId: string, resolution: string, userId: string) {
    const { data: flag } = await this.supabase.from('member_risk_flags').select('*').eq('id', flagId).single()
    if (!flag) throw new NotFoundException('Risk flag not found')

    const { data: updated } = await this.supabase.from('member_risk_flags').update({ resolved_at: new Date().toISOString(), resolved_by: userId }).eq('id', flagId).select().single()
  ...
  async getMemberKycStatus(memberId: string) {
    ...
    const { data: riskFlags } = await this.supabase.from('member_risk_flags').select('*').eq('member_id', memberId).is('resolved_at', null)
  ...
  async getMembersRequiringReview() {
    const { data } = await this.supabase.from('members').select('*').or('kyc_status.eq.pending,kyc_status.eq.failed,risk_score.gte.70,pep_status.eq.true').order('risk_score', { ascending: false })
    return data || []
  }
